...
public class CourseReviewController {
    ...
    @GetMapping("/{courseId}")
    public ResponseEntity<List<CourseReview>> getReviewsByCourseId(@PathVariable("courseId") long courseId) {
        try {
            List<CourseReview> list = courseReviewService.getReviewsByCourseId(courseId);
            return ResponseEntity.ok(list);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
